...
function handleGetAll(request,response,next) {
  request.model.get()
    ...
    .then(Q.publish('database','read', `Read all ${request.params.model}`))
    .catch( next );
}
...
function handleGetOne(request,response,next) {
  request.model.get(request.params.id)
    ...
    .then(Q.publish('database','read', `Read one ${request.params.model}`))
    .catch( next );
}
...
function handlePost(request,response,next) {
  request.model.post(request.body)
    ...
    .then(Q.publish('database','create', `Created a(n) ${request.params.model}`))
    .catch( next );
}
...
function handlePut(request,response,next) {
  request.model.put(request.params.id, request.body)
    .then( result => response.status(200).json(result) )
    .then(Q.publish('database','update', `Updated a(n) ${request.params.model}`))
    .catch( next );
}
...
function handleDelete(request,response,next) {
  request.model.delete(request.params.id)
    .then( result => response.status(200).json(result) )    
    .then(Q.publish('database','delete', `deleted a(n) ${request.params.model}`))
    .catch( next );
}
